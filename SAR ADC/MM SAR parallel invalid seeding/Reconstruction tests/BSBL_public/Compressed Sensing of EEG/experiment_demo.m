...
results_meanMSE = zeros(length(K_array),length(C_factor_array));
results_meanSSIM = zeros(length(K_array),length(C_factor_array));
results_meanTime = zeros(length(K_array),length(C_factor_array));
results_totalCap = zeros(length(K_array),length(C_factor_array));
...
%         Phi = load("sensing192").corrected_Phi;
        ...
        for ch = 1 : 1       % channel number
            for ep = 1 : 80     % epoch number
                ...
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                blkLen = 15;                  % the block size in the user-defined block partition
...
%         save(filename,"recon_EEG", "mse_bsbl", "ssim_bsbl", "t_bsbl");
        ...
        results_meanMSE(i,j) = mean(mse_bsbl);
        results_meanSSIM(i,j) = mean(ssim_bsbl);
        results_meanTime(i,j) = mean(t_bsbl);

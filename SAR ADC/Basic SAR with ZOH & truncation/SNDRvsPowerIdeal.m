...
for i=1:12
    ...
    comparator_noise_on = 0;
    comparator_noise_rms = 130*10^-4;
...
hold on
set(gca,'fontsize', 25);
scatter(SNDRi, totpoweri, 100, 'filled','DisplayName', 'total power');
scatter(SNDRi, comppoweri, 100, 'filled','DisplayName', 'comp power');
scatter(SNDRi, sarpoweri, 100, 'filled','DisplayName', 'sar power');
scatter(SNDRi, LNApoweri, 100, 'filled','DisplayName', 'lna power');
scatter(SNDRi, dacpoweri, 100, 'filled','DisplayName', 'dac power');
scatter(SNDRi, logicpoweri, 100, 'filled','DisplayName', 'logic power');
scatter(SNDRi, transmissionpoweri, 100, 'filled','DisplayName', 'transmission power');
title("Power vs SNDR (ideal)");
xlabel("SNDR (dB)");
ylabel("Power (W)");
legend
hold off
...
x = input_vector(4+2*number_of_bits:size(input_vector,1));
y = zeros(round((size(x,1)-2)/(2*(number_of_bits+1))),1);
i=1;
for j = 1:size(y,1)
    y(j) = x(i);

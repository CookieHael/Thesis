...
sigma_mismatch = .20;
pd = makedist('Normal', 1, sigma_mismatch);
rng(42069);

C1_array = minimum_cap*pd.random;
C2_array = cap_ratio*minimum_cap*pd.random(nb_channels,1);

%% MAC non-idealities & noise
C_sense_size = 1;
MM_incomplete_transfer_coeff = C1_array./(C1_array+C2_array);
if (non_idealities_on)
    MM_attenuation_coeff = C2_array./(C1_array+C2_array);  %Badly chosen name: this is voltage attenuation by charge sharing architecture
else
    MM_attenuation_coeff = 1;  %Actually more like 1-leakage
end

MM_sampling_noise = mean(noise_on*((k*T * C1_array./(C1_array+C2_array).^2) + (k*T*(C1_array.*C2_array./(C1_array+C2_array))./C2_array.^2)));  % Formula with square root is sigma, model uses variance
...
if (cap_ratio ~=35)
    sensing_matrix_corrected = rescaleSRBM_mismatched(sensing_matrix_large, C1_array, C2_array);
end
...
%% Total gain check
total_gain = LNA_gain*sampling_gain*1/(1+cap_ratio);
...
MM_attenuation_coeff = MM_attenuation_coeff(sensing_order,:);
MM_incomplete_transfer_coeff = MM_incomplete_transfer_coeff(sensing_order,:);

MM_attenuation_coeff_reshaped = repelem(MM_attenuation_coeff, nb_MAC)';
MM_incomplete_transfer_coeff_reshaped = repelem(MM_incomplete_transfer_coeff, nb_MAC)';
...
sim_out = sim('MMSAR_SequentialMismatched', (nb_channels+1)*clk_period*nb_MAC);
...
cs_out_fixed = 1/cap_ratio * cs_out ./ (C1_array./C2_array);
...
recovery_fixed = BSBL_BO(A, cs_out_fixed, 1:15:nb_MAC, 0, 'prune_gamma',-1, 'max_iters',10);
recovered_signal_fixed = (idct(recovery_fixed.x))';
recovered_signal_fixed = recovered_signal_fixed(1:nb_MAC);
...
[rms_out_fixed, mse_out_fixed] = calculateRMS(ADC_input*total_gain, recovered_signal_fixed);
mse_out_fixed
[rms_out_expected_fixed, mse_out_expected_fixed] = calculateRMS(expected_recovered_signal, recovered_signal_fixed);
...
plot(recovered_signal_fixed)
plot(total_gain*ADC_input)
hold off
legend("Recovered values without full mismatch correction","Recovered values with full mismatch correction" , "Input values");
title("Gained performance by gain correction: MSE = " + (mse_out - mse_out_fixed) + ". Final MSE: " + mse_out_fixed + " & SSIM = "+ssim_1)
subplot(222)
hold on
...
title("Digital recovery vs analog recovery, MSE = " + mse_out_expected_fixed)
...
legend("Digital coefficients", "Analog coefficients with gain correction", "Analog coefficients without gain correction");
subplot(224)
hold on
plot(expected_coeff-cs_out)
plot(expected_coeff-cs_out_fixed)
hold off
legend("Expected - coeff", "Expected - gain-corrected coeff")
